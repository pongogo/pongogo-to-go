...
FRICTION_PATTERNS = {
    "correction": [
        ...
        # GT v4 analysis patterns (restored from durian-0.6.2)
...
EXTENDED_FRICTION_PATTERNS = {
    "correction": [
        # F1: Frustration patterns - expectation mismatch indicators
        r"yet\s+another\s+example\s+of",  # repeated issue indicator
        r"isn'?t\s+this\s+critical",  # questioning importance/priority
        r"i\s+thought\s+we\s+(changed|fixed|updated)",  # expectation mismatch
        r"i\s+don'?t\s+think\s+you'?re\s+right",  # direct disagreement
        # F2: Process violation patterns
        r"without\s+going\s+through\s+(the\s+)?(proper\s+)?process",  # process bypass
        r"this\s+was\s+a\s+(breaking|critical)\s+change",  # severity complaint
        r"just\s+began.*without\s+(asking|checking)",  # premature action
        # F4: Soft correction patterns - implicit dissatisfaction
        r"seems\s+redundant",  # questioning necessity
        r"this\s+(final|last)\s+(line|part)",  # pointing out specific issues
        r"don'?t\s+bother\s+(creating|making|adding)",  # discouraging action
    ],
    "retry": [
        # F3: Code block error detection - retry friction with output
        r"```[\s\S]{0,500}error[\s\S]{0,500}```",  # error in code block
        r"```[\s\S]{0,500}failed[\s\S]{0,500}```",  # failed in code block
        r"⎿\s*(failed|error)",  # Claude Code output indicator
        r"(i\s+)?(got|see|seeing|showing)\s*(this|the)?\s*:\s*\n?```",  # output prefix + code block
    ],
}
...
COMPILED_ACTION_PATTERNS = dict(ACTION_REQUEST_PATTERNS.items())
...
class GuidanceFulfillmentTracker:
    ...
    def __init__(self, session_id: str, conversation_id: str | None = None):
        """
    ...
    def check_fulfillment(
        self, event_id: int, agent_response: str
    ) -> list[dict[str, Any]]:
        """
        ...
        self.pending_guidance = [
            g for g in self.pending_guidance if g["status"] == "pending"
        ]
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        if self.fulfilled_guidance:
            avg_distance = sum(g["distance"] for g in self.fulfilled_guidance) / len(
                self.fulfilled_guidance
            )
...
def _load_custom_guidance_triggers(
    ...
    try:
        ...
        if "explicit" in data:
            ...
            logger.info(
                f"Loaded {len(custom['explicit'])} custom explicit guidance triggers"
            )

        if "implicit" in data:
            custom["implicit"] = set(data["implicit"])
            logger.info(
                f"Loaded {len(custom['implicit'])} custom implicit guidance triggers"
            )
...
class RuleBasedRouter(RoutingEngine):
    ...
    def reload_lexicon(self):
        ...
        if self.features.get("use_lexicon"):
            self._load_lexicon()
            logger.info("Guidance trigger lexicon reloaded")
    ...
    def route(self, message: str, context: dict | None = None, limit: int = 5) -> dict:
        ...
        try:
            ...
            if (
                ...
                extended_friction_info = self._detect_extended_friction(message)
            ...
            for instruction in self.instruction_handler.instructions.values():
                ...
                if violation_info["detected"] and self.features.get(
                    ...
                    for checklist_inst in VIOLATION_CHECKLIST_BOOST.get(
                        "instructions", []
                    ):
                        if (
                            checklist_inst in inst_filename
                            or inst_filename in checklist_inst
                        ):
                            ...
                            score_breakdown["violation_checklist_boost"] = {
                                ...
                                "boost": boost_amount,
                                "violation_signals": violation_info.get("signals", [])[
                                    :3
                                ],  # First 3 signals
                            }
                            logger.debug(
            ...
            if guidance_info["detected"] and self.features.get(
                "friction_risk_watch", True
            ):
                friction_risk_watch = {
                    "enabled": True,
                    "guidance_type": guidance_info["guidance_type"],
                    "guidance_signals": guidance_info["signals"],
                    "echo_detected": guidance_echo["echo_detected"]
                    if guidance_echo
                    else False,
                    "frustration_level": guidance_echo["frustration_level"]
                    if guidance_echo
                    else "none",
    ...
    def _detect_friction(self, message: str) -> dict[str, Any]:
        ...
        - refinement: Course correction ("actually", "wait", "hold on")
        ...
        Returns:
            ...
            - friction_type: Type of friction or None
    ...
    def _detect_friction_db(self, message: str, entries: list) -> dict[str, Any]:
        """
    ...
    def _detect_friction_patterns(self, message: str) -> dict[str, Any]:
        """
        Pattern-based friction detection (original implementation).

        Uses hardcoded COMPILED_FRICTION_PATTERNS.
        """
        signals = []
        friction_type = None
    ...
    def _detect_user_guidance_db(self, message: str) -> dict[str, Any]:
        ...
        result = match_all_entries(self._lexicon_entries, message)
        ...
        best_match = max(result.triggered, key=lambda m: m.final_confidence)
        signals = []
    ...
    def _detect_user_guidance_patterns(self, message: str) -> dict[str, Any]:
        """
        Pattern-based user guidance detection (original implementation).

        Uses hardcoded EXPLICIT_GUIDANCE_TRIGGERS and IMPLICIT_GUIDANCE_TRIGGERS.
        """
        signals = []
        guidance_type = None
    ...
    def _detect_guidance_adherence(self, agent_response: str) -> dict[str, Any]:
        ...
        for category, pattern in COMPILED_COMPLIANCE_PATTERNS.items():
            ...
            if match:
                ...
                matches.append(
                    {"category": category, "matched_text": match.group()[:50]}
                )
                logger.debug(
    ...
    def _assess_friction_risk(self, guidance_adherence: str) -> dict[str, Any]:
        ...
        assessment = RISK_ASSESSMENT.get(
            guidance_adherence, RISK_ASSESSMENT["not_applicable"]
        )
    ...
    def _attribute_friction_cause(self, lookback_messages: int = 5) -> dict[str, Any]:
        """
        ...
        for db_path in db_paths:
            if db_path.exists():
                try:
                    ...
                    if rows:
                        logger.debug(
                            f"Phase 8: Scanned {len(rows)} prior events for friction attribution"
                        )

                except Exception as e:
                    logger.warning(
                        f"Phase 8: Error querying DB for friction attribution: {e}"
                    )
                    continue
    ...
    def _detect_guidance_echo(
        self, current_guidance_type: str | None, threshold: int = 3
    ) -> dict[str, Any]:
        """
        ...
        if count >= 5:
            ...
            recommendation = (
                "User repeating direction - ensure full compliance this time"
            )
    ...
    def _detect_extended_friction(self, message: str) -> dict[str, Any]:
        """
        ...
        Pattern sets:
        - F1: Frustration patterns (expectation mismatch)
        - F2: Process violation patterns
        - F3: Code block error detection (retry with output)
        - F4: Soft correction patterns (implicit dissatisfaction)
        ...
        Returns:
            Dictionary with same structure as _detect_friction:
            - detected: True if friction pattern found
            - friction_type: 'correction' or 'retry'
            - signals: List of matched patterns
            - category_boosts: Categories to boost
        """
        signals = []
