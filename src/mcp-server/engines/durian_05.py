...
COMMENCEMENT_PHRASES = [
    "let's continue", "let's proceed", "let's resume", "let's go ahead",
    "let's get started", "let's begin", "let's start",
    "please continue", "please proceed", "please resume", "please go ahead",
    "yes, let's continue", "yes, let's proceed", "yes, let's resume",
    "yes, let's begin", "yes, let's start", "yes, please continue",
    "yes, please proceed", "go ahead", "go ahead and continue",
...
VIOLATION_WORDS = {
    ...
    'violation', 'violate', 'breach', 'sloppy', 'careless', 'shortcuts',
}
...
class DurianRouter05(RoutingEngine):
    ...
    def __init__(self, handler: InstructionHandler, features: Optional[Dict] = None):
        ...
        logger.info(f"DurianRouter05 initialized (version: {self.version}, features: {feature_str})")
    ...
    def route(self, message: str, context: Optional[Dict] = None, limit: int = 5) -> Dict:
        ...
        try:
            ...
            scored_instructions = []
            analysis = {
                'keywords_extracted': keywords, 'intent_detected': intent,
                'context_used': context, 'features': self.features,
            ...
            for instruction in self.instruction_handler.instructions.values():
                score, score_breakdown = self._score_instruction(
                    instruction=instruction, message=message, keywords=keywords,
                    intent=intent, files=files, directories=directories,
                    branch=branch, language=language,
                    violation_info=violation_info, semantic_flags_info=semantic_flags_info
                )
                ...
                if score > 0:
                    ...
                    analysis['scoring_breakdown'].append({
                        ...
                        'score': score, 'breakdown': score_breakdown
                    })
            ...
            if self.features.get('foundational', True):
                ...
                query_specific = [inst for inst in scored_instructions[:limit]
            ...
            return {'instructions': combined, 'count': len(combined), 'routing_analysis': analysis}
    ...
    def _score_instruction(self, instruction, message: str, keywords: List[str],
                          intent: str, files: List[str], directories: List[str],
                          branch: str, language: str,
                          violation_info: Optional[Dict] = None,
                          semantic_flags_info: Optional[Dict] = None) -> tuple[int, Dict]:
            ...
            for category in instruction.categories:
                if category in VIOLATION_BOOST_CATEGORIES:
                    ...
                    breakdown['violation_boost'] = {
                        'category': category, 'boost': violation_info['boost_amount'],
                            ...
                            for partner_inst in scored_instructions:
                                ...
                                if partner_id == p_id or partner_id == p_normalized:
                                    ...
                                    partner_inst['score_breakdown']['bundle_boost'] = {
                                        'from': id_format, 'boost': boost_amount,
                                        'co_occurrence_rate': co_occurrence_rate
                                    }
                                    boosts_applied.append({
                                        'trigger': id_format, 'boosted': partner_id,
                                        'amount': boost_amount
                                    })
                                    break
                    break

        return {'applied': len(boosts_applied) > 0, 'boosts': boosts_applied, 'total_boosts': len(boosts_applied)}
